...
for (;;) {
    ...
    try {
        ...
        $containers = $driver->wait(10, 1000)->until(
            WebDriverExpectedCondition::presenceOfAllElementsLocatedBy(WebDriverBy::xpath("/html/body/div[1]/div/div[4]/div"))
        );
        if(is_array($containers) && count($containers) > 0) {
            ...
            try {
                function closeWindow() {
                    $selectors = [
                        WebDriverBy::cssSelector('.modal button.close'),
                        WebDriverBy::cssSelector('div[role="dialog"] button[aria-label="Close"]'),
                        WebDriverBy::cssSelector('.modal button[aria-label="Close"]'),
                        WebDriverBy::cssSelector('div.popup-close'),
                        WebDriverBy::xpath('//button[contains(text(), "×")]'),
                        WebDriverBy::xpath("//button[contains(text(), 'إغلاق')]"),
                    ];

                    foreach ($selectors as $selector) {
                        try {
                            $closeBtn = $driver->findElement($selector);
                            $closeBtn->click();
                        } catch (Exception $e) {
                            log_stat("no selector");
                        }
                    }
                }
                foreach ($containers as $index => $container) {
                    ...
                    try {
                        $link = $container->findElement(WebDriverBy::xpath('./div/a'))->getAttribute('href');
                        $init_title = $container->findElement(WebDriverBy::xpath('./div/a'))->getAttribute('title');
                        function extractBookName($text) {
                            $pattern = '/(?:كتاب|رواية)\s+(.*?)\s+تأليف/';
                            if (preg_match($pattern, $text, $matches)) {
                                return $matches[1]; // The book name
                            }
                            return null; // No match found
                        }
                        $title = extractBookName($init_title);
                        if($title) {
                            ...
                            try {
                                ...
                                try {
                                    $pageElem = $driver->findElement(WebDriverBy::xpath(
                                        "//span[contains(text(), 'عدد الصفحات')]"
                                    ));

                                    if ($pageElem) {
                                        $book_pages = $driver->executeScript("return arguments[0].parentElement.nextElementSibling.innerText", [$pageElem]);
                                        if($book_pages) {
                                    ...
                                    log_stat("pages" . $e->getMessage());
                                }
                                // author
                                try {
                                    $authorElem = $driver->findElement(WebDriverBy::xpath(
                                        "//span[contains(text(), 'اسم المؤلف')]"
                                    ));
                                    if ($authorElem) {
                                        $book_author = $driver->executeScript("return arguments[0].parentElement.nextElementSibling.innerText", [$authorElem]);
                                    }
                                } catch (Exception $e) {
                                    log_stat("author" . $e->getMessage());
                                }
                                // image download
                                $image_src = NULL;
                                try {
                                    $image = $driver->findElement(WebDriverBy::xpath('/html/body/div[1]/div/div[2]/div[1]/div[1]/div[1]/a/img'))->getAttribute('src');
                                    ...
                                    if($image) {
                                        ...
                                        log_stat("images else" . $e->getMessage());
                                        goto skip_image;
                                    }
                                } catch (Exception $e) {
                                    log_stat("images catch" . $e->getMessage());
                                    goto skip_image;
                                }
                                skip_image:
                                ...
                                if($datasql['book_without_pdf'] == 'off') {
                                    try {
                                        /*sleep(3);
                                        closeWindow();*/
                                        $svg = $driver->findElement(WebDriverBy::cssSelector('.inline-block.m-2.w-6.h-6.fill-current'));
                                        $btn = $driver->executeScript("return arguments[0].parentElement", [$svg]);
                                        $btn->click();
                                        sleep(1);
                                        $wait = new WebDriverWait($driver, 35); // Wait up to 30 seconds
                                        $wait->until(function($driver) {
                                            $element = $driver->findElement(WebDriverBy::cssSelector('div.bg-sugar-800'));
                                            $display = $element->getCssValue('width');
                                            return $display === '100%'; // Wait until display is 'none'
                                        });
                                        sleep(1);
                                        $btn = $driver->findElement(WebDriverBy::xpath("/html/body/div[1]/div/div[2]/div[1]/div[4]/div/div/div/div[2]/div/div/div/div/a"));
                                        if($btn) {
                                ...
                                if (!empty($title)) {
                                    ...
                                    if ($insert) {
                                        ...
                                        log_stat("post not inserted");
                                        $fails++;
                                    }
                                } else {
                                    log_stat("empty title");
                                    $fails++;
                                }
