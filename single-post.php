...
$reviewed = get_post_field($post_id, 'reviewed');
...
<body style="background-color: #f4f4f4 !important;">
	...
	<div class="container">
		...
		<!-- modal of raters -->
		<div class="modal fade" id="RaterModal" tabindex="-1" role="dialog" aria-labelledby="RaterModalLabel">
			<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
					<div class="modal-content">
							<div class="modal-header">
									<h5 class="modal-title" id="RaterModalLabel">قَيِّموا هذا المنشور</h5>
									<button type="button" class="close" data-dismiss="modal" aria-label="Close">
											<span>&times;</span>
									</button>
							</div>
							<div class="modal-body" id="userList">
								<?php
								$users = $dsql->dsql()
								->expr("SELECT users.*, rating_sys.id as r_id FROM users INNER JOIN rating_sys ON users.id = rating_sys.user_id WHERE rating_sys.post_id = $post_id;")
								->get();
								?>
									<?php if ($users): ?>
											<?php foreach ($users as $user): ?>
													<div class="user-item mb-3 py-2" data-user-id="<?= $user['id']; ?>">
		...
		<!-- end of modal of raters -->

		<div class="modal fade" id="commentModal" tabindex="-1" role="dialog" aria-labelledby="commentModalLabel">
			<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
					<div class="modal-content">
							<div class="modal-header">
									<h5 class="modal-title" id="commentModalLabel">شارك مشاعرك بكتابة تعليق على المنشور (اختياري)</h5>
									<button type="button" class="close" data-dismiss="modal" aria-label="Close">
											<span>&times;</span>
									</button>
							</div>
								...
								?>
									...
									<div class="comments">
										<div class="add-comment-form">
											<form action="" method="POST" id="comment-form-2" class="<?php echo $class_attr; ?>">
												<div class="form-row">
													<div class="col-12">
														<div class="comment-form">
															<div>
																<div class="d-flex" style="height: 155px;">
																	<img src="<?php echo $user_pic; ?>" class="user-pic img-fluid mr-1" />
																	<textarea style="z-index: 3; direction: rtl;" id="emojoComment-33" name="comment" class="form-control rounded h-100"></textarea>
												...
												<input type="hidden" name="comment_attachment" value="" />
									...
									<script>
										...
										if($("#emojoComment-33").length > 0) {
											var emojioneArea = $("#emojoComment-33").emojioneArea({
												...
												events: {
													...
													keyup: function (editor, event) {
														if (event.key === "Enter" && !event.shiftKey) {  // Si l'utilisateur appuie sur "Entrée" sans majuscule
															event.preventDefault(); // Empêche l'ajout d'un saut de ligne
															sendComment2();  // Appelle la fonction pour envoyer le commentaire
														}
													}
												}
											});
											...
											function sendComment2() {
												...
												if (comment.trim() !== "") { // Vérifie que le commentaire n'est pas vide
													...
													$("#emojoComment-33").parents('form').find('.add-comment').click();
												}
											}
										}
									</script>
	...
	<div class="container">
		<div class="d-sm-flex justify-content-center flex-wrap">
			...
			<?php if ($post["post_type"] == "book") : ?>
				...
				$share_authority = get_post_field($post_id, 'share_authority');
				$preview_stat = true;
				$download_stat = true;

				if($share_authority == 'on') {
					$h4 = 'تم سحب حق النشر <img style="width: 19px" src="'. siteurl() . "/assets/images/icons/book/book02.svg" .'" />';
					$span = 'حفاظًا على الحقوق، تم تعطيل القراءة والتحميل';
					$display_book_success_alert = true;
					$preview_stat = false;
					$download_stat = false;
				} else {
					if($is_for_read == "on") {
						$h4 = 'الكتاب للمراجعة فقط <img style="width: 19px" src="'. siteurl() . "/assets/images/icons/book/book03.svg" .'" />';
						$span = 'الكتاب مخصص للتقييم وإبداء الرأي';
						$display_book_success_alert = true;
						$preview_stat = false;
						$download_stat = false;
					} else {
						if($is_book_author == 1) {
							$h4 = 'نشره المؤلف/المترجم <img style="width: 19px" src="'. siteurl() . "/assets/images/icons/book/book04.svg" .'" />';
							$span = 'الكتاب مخصص للقراءة والتحميل غير التجاري';
							$display_book_success_alert = true;
							$preview_stat = true;
							$download_stat = true;
						} else {
							$h4 = 'هذا الكتاب ملكية عامة <img style="width: 19px" src="'. siteurl() . "/assets/images/icons/book/book01.svg" .'" />';
					...
					<?php if (is_array($get_ads_top_big)): foreach ($get_ads_top_big as $ad_2): update_ad_views($ad_2["ad_key"]); ?>
						...
						?>
							...
							<div class="sidebar-box sidebar-box-rating border-0">
								<h3 class="text-center mt-3 mb-0">قيم المنشور</h3>
								<div class="rating">
								...
								?>
								<h5 class="text-muted text-center">اختر تقييمًا لعرض رأيك</h5>
								<div class="row w-100 mx-auto my-3 justify-content-center align-items-center text-muted">
									<span>
										<a class="" data-toggle="modal" data-target="#RaterModal" href="#">التقييمات</a>
										<span class="badge badge-warning"><?= $total_ratings ;?></span>
									</span>
									<div class="vr mx-2"></div>
									<span>
										معدل التقييمات
										<span class="badge badge-warning"><?= $average_rating ;?></span>
									</span>
								</div>
...
<!-- dfnsdfskdlflsmdfkslmdfkdslmfkdsmlk -->
			</div>
			<div class="my-5"></div>
		...
		document.addEventListener("DOMContentLoaded", () => {
				const modalBody = document.querySelector("#RaterModal .modal-body");
				let loading = false; // Prevent multiple calls
				let page = 1;

				modalBody.addEventListener("scroll", () => {
					if (!loading && modalBody.scrollTop + modalBody.clientHeight >= modalBody.scrollHeight - 10) {
						loading = true;
						page++;

						// Fetch additional user data
						fetch(`/user-ajax.php?post_id=<?= $post_id; ?>&page=${page}`)
						...
						.then(data => {
							if (data.length > 0) {
								...
								data.forEach(user => {
									...
									userDiv.classList.add("user-item", "mb-3");
									userDiv.innerHTML = `
